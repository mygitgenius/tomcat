...
<%
	...
	Object[] result = ActionLogDAO.getInstance().getLogList("moderator", reason, pageno, resultCount);
%>
      ...
      <TABLE class=guide cellSpacing=0 cellPadding=0 width="100%" border=0>
        ...
        <TR>
          <TD><A onclick="parent.location='../index.htm'; return false;" 
            href="#">后台管理首页</A>&nbsp;&raquo;&nbsp;版主管理记录</TD></TR>
	  </TBODY></TABLE><BR>
	<FORM id="frmsearch" name="frmsearch" onSubmit="checkfields(this); return false;" action="./log_moderator.jsp" method=post>
	<input type="hidden" name="page" value="<%= pageno %>">
      <TABLE class=info_tb cellSpacing=0 cellPadding=0 width="100%" border=0>
        <TBODY>
        <TR class=header>
          <TD colSpan=3>版主管理记录&nbsp;-&nbsp; 过滤条件</TD></TR>
...
%>	  
        <TR align=middle>
          ...
          <TD class=altbg2><%= (String)record.get("GROUPNAME") %></TD>
          <TD class=altbg1><%= (String)record.get("REMOTEIP") %></TD>
          ...
          <TD class=altbg1><A href="<%= topicURL %>" target=_blank><%= topicTitle %></A></TD>
          <TD class=altbg2><%= (String)record.get("ACTION") %></TD>
          <TD class=altbg1><%= (String)record.get("REASON") %>&nbsp;</TD></TR>
<%
		}
	}
%>		  
