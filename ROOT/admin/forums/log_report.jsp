...
<%
	...
	Object[] result = ActionLogDAO.getInstance().getLogList("report", reason, pageno, resultCount);
%>
      ...
      <TABLE class=guide cellSpacing=0 cellPadding=0 width="100%" border=0>
        ...
        <TR>
          <TD><A onclick="parent.location='../index.htm'; return false;" 
            href="#">后台管理首页</A>&nbsp;&raquo;&nbsp;用户举报记录</TD></TR></TBODY></TABLE><BR>
	<FORM id="frmsearch" name="frmsearch" onSubmit="checkfields(this); return false;" action="./log_report.jsp" method=post>
	<input type="hidden" name="page" value="<%= pageno %>">
      <TABLE class=info_tb cellSpacing=0 cellPadding=0 width="100%" border=0>
        <TBODY>
        <TR class=header>
          <TD colSpan=3>用户举报记录&nbsp;-&nbsp; 过滤条件</TD></TR>
		...
		<TR class="altbg2"><TD width="25%">原因(可使用通配符 *):</TD>
			<TD width="55%"><input type="text" name="reason" size="50" maxlength="20"></TD>
			<TD width="20%"><input class="button" type="submit" value="提 交"></TD></TR>
        </TBODY></TABLE><BR>
	  </FORM>
<%
...
%>
      <TABLE class=info_tb cellSpacing=0 cellPadding=0 width="100%" border=0>
        ...
        <TR class=header>
          <TD width="12%">用户名</TD>
          <TD width="12%">被举报用户</TD>
          <TD width="15%">时间</TD>
          <TD width="13%">版块</TD>
          <TD width="33%">标题</TD>
          <TD width="15%">原因</TD></TR>
<%
	if (result != null && result[1] != null)
	{
		...
		for (int i=0; i<logList.size(); i++)
		{
			...
			reportedUser = (String)record.get("REPORTEDUSER");
...
%>	  
        <TR align=middle>
          ...
          <TD class=altbg2><A href="../../uspace.jsp?uid=<%= reportedUser %>" 
			   target=_blank><%= reportedUser.length()==0?"游客":reportedUser %></A></TD>
          <TD class=altbg1><%= AppUtils.formatSQLTimeStr((String)record.get("CREATETIME")) %></TD>
          <TD class=altbg2><A href="../../forum.jsp?fid=<%= (String)record.get("BOARDID") %>" 
                target=_blank><%= (String)record.get("BOARDNAME") %></A></TD>
          <TD class=altbg1>
		  	<A href="<%= topicURL %>" target=_blank><%= topicTitle %></A></TD>
          <TD class=altbg2><%= (String)record.get("REASON") %>&nbsp;</TD>
		</TR>
<%
		}
	}
